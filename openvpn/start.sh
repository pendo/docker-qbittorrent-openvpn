...
if [[ ! -z "${VPN_ENABLED}" ]]; then
	echo "[info] VPN_ENABLED defined as '${VPN_ENABLED}'"
else
	echo "[warn] VPN_ENABLED not defined,(via -e VPN_ENABLED), defaulting to 'yes'"
...
if [[ $VPN_ENABLED == "yes" ]]; then
	...
	cp /etc/openvpn/openvpn.ovpn /config/openvpn/openvpn.ovpn
	cp /etc/openvpn/credentials.conf /config/openvpn/credentials.conf
	...
	if [[ ! -z "${VPN_PROTOCOL}" ]]; then
		...
		if [[ ! -z "${VPN_PROTOCOL}" ]]; then
			...
			echo "[warn] VPN_PROTOCOL not found in ${VPN_CONFIG}, assuming udp"
	...
	if [[ ! -z "${LAN_NETWORK}" ]]; then
		echo "[info] LAN_NETWORK defined as '${LAN_NETWORK}'"
	else
	...
	if [[ ! -z "${NAME_SERVERS}" ]]; then
		...
		echo "[warn] NAME_SERVERS not defined (via -e NAME_SERVERS), defaulting to Google and FreeDNS name servers"
	...
	if [[ ! -z "${VPN_OPTIONS}" ]]; then
		echo "[info] VPN_OPTIONS defined as '${VPN_OPTIONS}'"
	else
		echo "[info] VPN_OPTIONS not defined (via -e VPN_OPTIONS)"
	fi
elif [[ $VPN_ENABLED == "no" ]]; then
	echo "[warn] !!IMPORTANT!! You have set the VPN to disabled, you will NOT be secure!"
fi
...
if [[ -z "${PUID}" ]]; then
	echo "[info] PUID not defined. Defaulting to root user"
	export PUID="root"
fi

if [[ -z "${PGID}" ]]; then
	echo "[info] PGID not defined. Defaulting to root group"
	export PGID="root"
fi
